...
const PaymentsChart = () => {
  ...
  const fetchData = () => {
    ...
    let p2 =axios.get(
      `https://innovit-2cs-project.onrender.com/dashboard/successPay/${ctxt.user.idEntreprise}`
    )
    .then((res) => {
      let t = []
      if (res.status == 200) {
        res.data.map((payment) => {
          let date = payment.date.slice(0,-2)+ "20"+payment.date.slice(-2)
          date = date.split("-").reverse().join("-")
          t.push({date: date, status:"success"})
        })
        setOrdersData([...ordersData, ...t])
      }
    })
    .catch((e) => {
      console.log(e)
    ...
    let p1 = axios.get(
      `https://innovit-2cs-project.onrender.com/dashboard/failedPay/${ctxt.user.idEntreprise}`
    )
    .then((res) => {
      let t = []
      if (res.status == 200) {
        res.data.map((payment) => {
          let date = payment.date.slice(0,-2)+ "20"+payment.date.slice(-2)
          date = date.split("-").reverse().join("-")
          t.push({date: date, status:"failed"})
        })
        setOrdersData([...ordersData, ...t])
      }
  ...
  const chartOptions1 = {
    plugins: {
      title: {
        display: true,
        text: 'Succesful/failed payments',
      },
    },
    scales: {
      x: {
